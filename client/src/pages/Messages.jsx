...
export default function Messages() {
  ...
  const { isLoading: isConversationsLoading, data: conversations } =
    useGetConversations();
  ...
  } = useGetConversationsMessages(conversationId, {
    initialData: { data: [] },
  });

  const { mutate: createMessageMutate, isSuccess: createMessageIsSuccess } =
    useCreateMessage();
  const {
    mutate: createConversationMutate,
    isSuccess: createConversationIsSuccess,
  } = useCreateConversation();
  ...
  const selectMembersHandler = (user) => {
    ...
    if (
      !selectedMembers.some((selectedUser) => selectedUser._id === user._id)
    ) {
  ...
  const createConversationHandler = () => {
    ...
    const existingConversation = conversations?.data?.data.find((conv) => {
      return memberIds.every(id => conv.members.map(member => member._id).includes(id));
  });
  ...
  const otherUser = conversation?.data?.data?.members.find(
                    ...
                    <>
                      {conversations.data.data.map((conversation) => {
                        ...
                        return (
                          <div
                            onClick={() =>
                              conversationHandler(conversation._id)
                            }
                            key={conversation._id}
                            className="cursor-pointer card px-5 py-2 hover:bg-gray-100 flex items-center gap-2"
                          >
                            <div
                              className="img w-12 h-12 bg-gray-400 rounded-full "
                              style={{
                  ...
                  <div className="flex-1 overflow-y-auto">
                    <div className="header flex fixed bg-white justify-between w-[60%] border-b border-gray-200 py-3 px-4">
                      <div className="flex items-center gap-3">
                        <div
                          className="img w-11 h-11 bg-gray-400 rounded-full"
                          style={{
                        ...
                        ></div>
                        <div className="flex flex-col">
                          <h2 className="text-gray-800 text-lg font-semibold leading-5">
                            {otherUser?.username}
                          </h2>
                    ...
                    <div className="h-full space-y-4 px-6 flex flex-col justify-between">
                      <div className="flex flex-col items-center mt-28">
                        <div
                          className="img w-32 h-32 bg-gray-400 rounded-full"
                          style={{
                            backgroundImage: `url("${otherUser?.profile?.picture}"), url("${fallbackImage}")`,
                            backgroundPosition: "center",
                            backgroundSize: "cover",
                    ...
                    <div className="border border-gray-300 rounded-3xl">
                      <div className=" px-5 py-1 flex items-center gap-3">
                        ...
                        <div className="flex items-center">
                          <span className="hover:cursor-pointer p-2">
                            <svg
        ...
        >
          <div className="w-[500px] h-[60vh] relative">
                  ...
                  {openChat ? (
                    <>
                      {selectedMembers?.map((member) => (
                        <div
                          key={member._id}
                          className="userName-Card flex items-center font-medium text-blue-500 bg-blue-100 rounded-full hover:cursor-pointer"
                        >
                          <span className="pl-4 pr-1 py-[2px] hover:text-gray-600">
                            {member.username}
                          </span>
              ...
              {isUsersLoading ? (
                ...
                <>
                  {openChat && search?.length > 0 ? (
                    <>
                      {users?.data?.users
                        .filter((user) => user._id !== currentUser.data._id)
                        .map((user) => (
                          <div
                            onClick={() => selectMembersHandler(user)}
                            key={user._id}
                          ...
                          >
                            <div className="cursor-pointer px-5 py-2 hover:bg-gray-100 transition duration-300 ease-in-out flex justify-between items-center">
                              ...
                              <div className="mr-2 flex items-center justify-center h-6 w-6 border-2 border-gray-500 rounded-full"></div>
                            </div>
                          </div>
                        ))}
                    </>
            ...
            <div
              ...
              className="button absolute bottom-0  w-full bg-white py-3 px-4"
            >
              <button className="bg-blue-500 hover:bg-blue-600 text-white py-3 w-full rounded-lg">
                Chat
              </button>
            </div>
          </div>
